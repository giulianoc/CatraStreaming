...
MP4CttsAtom:: MP4CttsAtom (void): MP4Atom ()
...
MP4CttsAtom:: MP4CttsAtom (const MP4CttsAtom &)
...
MP4CttsAtom &MP4CttsAtom:: operator = (const MP4CttsAtom &)
...
#ifdef WIN32
	Error MP4CttsAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4CttsAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4CttsAtom:: createProperties (
...

{
	...
	if (_mui32pEntries. init ("Entries", ulValue * 2, &bIsImplicitProperty,
		&ulValue, _ptTracer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_INIT_FAILED);
...
Error MP4CttsAtom:: readProperties (
	unsigned long *pulPropertiesSize)

{
	...
	// 3: Entries
	// 		sampleCount / sampleOffset
	if (_mui32pEntries. init ("Entries", ulValue * 2, _pfFile,
		_ptTracer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_INIT_FAILED);
...
Error MP4CttsAtom:: writeProperties (int iFileDescriptor)
...
Error MP4CttsAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)
...
Error MP4CttsAtom:: getType (char *pType)
...
Error MP4CttsAtom:: appendPropertiesDump (Buffer_p pbBuffer)
...
Error MP4CttsAtom:: printPropertiesOnStdOutput (void)
...
Error MP4CttsAtom:: getEntriesNumber (unsigned long *pulEntriesNumber)
...
Error MP4CttsAtom:: setEntriesNumber (unsigned long ulEntriesNumber)
...
Error MP4CttsAtom:: incrementEntriesNumber (long lIncrement)
...
Error MP4CttsAtom:: getEntries (unsigned long ulIndex,
	unsigned long *pulEntries)

{
	...
	if (_mui32pEntries. getValue (pulEntries, ulIndex) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_GETVALUE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4CttsAtom:: setEntries (unsigned long ulIndex,
...

{
	...
	if (_mui32pEntries. setValue (ulEntries, ulIndex,
		&ulSizeChangedInBytes) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_SETVALUE_FAILED);
...
Error MP4CttsAtom:: incrementEntries (unsigned long ulIndex,
	long lIncrement)

{
	...
	if (_mui32pEntries. incrementValue (ulIndex, lIncrement) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_INCREMENTVALUE_FAILED);
