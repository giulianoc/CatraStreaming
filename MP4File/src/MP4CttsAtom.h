...
#ifndef MP4CttsAtom_h
	#define MP4CttsAtom_h

	#include "MP4Atom.h"
	#include "MP4FileErrors.h"


	typedef class MP4CttsAtom: public MP4Atom {
		...
		private:
			...
			MP4UInteger32BitsProperty_t				_mui32pEntries;

		protected:
			MP4CttsAtom (const MP4CttsAtom &);

			MP4CttsAtom &operator = (const MP4CttsAtom &);
		...
		public:
			...
			*/
			...
			Error getEntries (unsigned long ulIndex,
				unsigned long *pulEntries);

			Error setEntries (unsigned long ulIndex,
				unsigned long ulEntries);

			Error incrementEntries (unsigned long ulIndex,
				long lIncrement);

	} MP4CttsAtom_t, *MP4CttsAtom_p;
