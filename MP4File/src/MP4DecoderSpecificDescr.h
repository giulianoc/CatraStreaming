...
#ifndef MP4DecoderSpecificDescr_h
	#define MP4DecoderSpecificDescr_h

	#include "MP4Descr.h"
	#include "MP4FileErrors.h"


	typedef class MP4DecoderSpecificDescr: public MP4Descr {

		private:
			MP4BytesProperty_t					_mbpInfo;

		protected:
			MP4DecoderSpecificDescr (
				const MP4DecoderSpecificDescr &);

			MP4DecoderSpecificDescr &operator = (
				const MP4DecoderSpecificDescr &);
		...
		public:
			...
			virtual ~MP4DecoderSpecificDescr ();
			...
			#ifdef WIN32
				Error getPointerToInfo (unsigned char **pucInfo,
			...
			Error setInfo (unsigned char *pucInfo,
				unsigned long ulInfoSize);

	} MP4DecoderSpecificDescr_t, *MP4DecoderSpecificDescr_p;
