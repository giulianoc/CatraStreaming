...
MP4ElstAtom:: MP4ElstAtom (void): MP4Atom ()
...
MP4ElstAtom:: MP4ElstAtom (const MP4ElstAtom &)
...
MP4ElstAtom &MP4ElstAtom:: operator = (const MP4ElstAtom &)
...
#ifdef WIN32
	Error MP4ElstAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4ElstAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4ElstAtom:: createProperties (
...

{
	...
	// 3: EditListTable
	// 		segmentDuration / mediaTime / mediaRate
	if (_mui32pEditListTable. init ("EditListTable", ulValue * 3,
...
Error MP4ElstAtom:: readProperties (
	unsigned long *pulPropertiesSize)

{
	...
	if (_mui32pEntriesNumber. getValue (&ulValue, 0) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4ElstAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui32pEditListTable. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_WRITE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4ElstAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mui32pEditListTable. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_GETSIZE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4ElstAtom:: getType (char *pType)
...
Error MP4ElstAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui32pEditListTable. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_APPENDDUMP_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4ElstAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui32pEditListTable. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_PRINTONSTDOUTPUT_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4ElstAtom:: getEntriesNumber (unsigned long *pulEntriesNumber)
...
Error MP4ElstAtom:: setEntriesNumber (unsigned long ulEntriesNumber)
...
Error MP4ElstAtom:: getEditListTable (unsigned long ulIndex,
	unsigned long *pulEditListTable)

{
	...
	if (_mui32pEditListTable. getValue (pulEditListTable, ulIndex) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_GETVALUE_FAILED);
...
Error MP4ElstAtom:: setEditListTable (unsigned long ulIndex,
	unsigned long ulEditListTable)

{
	...
	if (_mui32pEditListTable. setValue (ulEditListTable, ulIndex,
		&ulSizeChangedInBytes) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_SETVALUE_FAILED);
