...
#ifndef MP4ElstAtom_h
	#define MP4ElstAtom_h

	#include "MP4Atom.h"
	#include "MP4FileErrors.h"


	typedef class MP4ElstAtom: public MP4Atom {
		...
		private:
			...
			MP4UInteger32BitsProperty_t				_mui32pEditListTable;

		protected:
			MP4ElstAtom (const MP4ElstAtom &);

			MP4ElstAtom &operator = (const MP4ElstAtom &);
		...
		public:
			...
			Error getEditListTable (unsigned long ulIndex,
				unsigned long *pulEditListTable);

			Error setEditListTable (unsigned long ulIndex,
				unsigned long ulEditListTable);

	} MP4ElstAtom_t, *MP4ElstAtom_p;
