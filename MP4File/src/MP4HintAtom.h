...
#ifndef MP4HintAtom_h
	#define MP4HintAtom_h

	#include "MP4Atom.h"
	#include "MP4FileErrors.h"


	typedef class MP4HintAtom: public MP4Atom {

		private:
			MP4UInteger32BitsProperty_t			_mui32pEntriesNumber;
			MP4UInteger32BitsProperty_t			_mui32pTracksReferencesTable;

		protected:
			MP4HintAtom (const MP4HintAtom &);

			MP4HintAtom &operator = (const MP4HintAtom &);
		...
		public:
			...
			Error getTracksReferencesTable (unsigned long ulIndex,
				unsigned long *pulTracksReferencesTable);

			Error setTracksReferencesTable (unsigned long ulIndex,
				unsigned long ulTracksReferencesTable);

	} MP4HintAtom_t, *MP4HintAtom_p;
