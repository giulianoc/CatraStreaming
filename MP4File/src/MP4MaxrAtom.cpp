...
MP4MaxrAtom:: MP4MaxrAtom (void): MP4Atom ()
...
MP4MaxrAtom:: MP4MaxrAtom (const MP4MaxrAtom &)
...
MP4MaxrAtom &MP4MaxrAtom:: operator = (const MP4MaxrAtom &)
...
#ifdef WIN32
	Error MP4MaxrAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4MaxrAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4MaxrAtom:: createProperties (
...

{
	...
	// 0: granularity
	if (_mui32pGranularity. init ("Granularity",
...
Error MP4MaxrAtom:: readProperties (
	unsigned long *pulPropertiesSize)
...
Error MP4MaxrAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui32pGranularity. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mui32pBytes. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_WRITE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4MaxrAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					= 0;
	...
	if (_mui32pGranularity. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mui32pBytes. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_GETSIZE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4MaxrAtom:: getType (char *pType)
...
Error MP4MaxrAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui32pGranularity. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mui32pBytes. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_APPENDDUMP_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4MaxrAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui32pGranularity. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mui32pBytes. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_PRINTONSTDOUTPUT_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4MaxrAtom:: getGranularity (unsigned long *pulGranularity)

{
	...
	if (_mui32pGranularity. getValue (pulGranularity, 0) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4MaxrAtom:: setGranularity (unsigned long ulGranularity)

{
	...
	if (_mui32pGranularity. setValue (ulGranularity, 0,
...
Error MP4MaxrAtom:: getBytes (unsigned long *pulBytes)

{
	...
	if (_mui32pBytes. getValue (pulBytes, 0) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_GETVALUE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4MaxrAtom:: setBytes (unsigned long ulBytes)

{
	...
	if (_mui32pBytes. setValue (ulBytes, 0, &ulSizeChangedInBytes) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_SETVALUE_FAILED);
