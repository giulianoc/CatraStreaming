...
#ifndef MP4MaxrAtom_h
	#define MP4MaxrAtom_h

	#include "MP4Atom.h"
	#include "MP4FileErrors.h"


	typedef class MP4MaxrAtom: public MP4Atom {

		private:
			MP4UInteger32BitsProperty_t				_mui32pGranularity;
			MP4UInteger32BitsProperty_t				_mui32pBytes;

		protected:
			MP4MaxrAtom (const MP4MaxrAtom &);

			MP4MaxrAtom &operator = (const MP4MaxrAtom &);
		...
		public:
			...
			Error getGranularity (unsigned long *pulGranularity);

			Error setGranularity (unsigned long ulGranularity);

			Error getBytes (unsigned long *pulBytes);

			Error setBytes (unsigned long ulBytes);

	} MP4MaxrAtom_t, *MP4MaxrAtom_p;
