...
MP4MfhdAtom:: MP4MfhdAtom (void): MP4Atom ()
...
MP4MfhdAtom:: MP4MfhdAtom (const MP4MfhdAtom &)
...
MP4MfhdAtom &MP4MfhdAtom:: operator = (const MP4MfhdAtom &)
...
#ifdef WIN32
	Error MP4MfhdAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4MfhdAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4MfhdAtom:: createProperties (
...

{
	...
	// 2: sequenceNumber
	if (_mui32pSequenceNumber. init ("SequenceNumber",
...
Error MP4MfhdAtom:: readProperties (
	unsigned long *pulPropertiesSize)
...
Error MP4MfhdAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui32pSequenceNumber. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4MfhdAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mui32pSequenceNumber. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4MfhdAtom:: getType (char *pType)
...
Error MP4MfhdAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui32pSequenceNumber. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4MfhdAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui32pSequenceNumber. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
