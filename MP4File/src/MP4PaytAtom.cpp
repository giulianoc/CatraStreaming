...
MP4PaytAtom:: MP4PaytAtom (void): MP4Atom ()
...
MP4PaytAtom:: MP4PaytAtom (const MP4PaytAtom &)
...
MP4PaytAtom &MP4PaytAtom:: operator = (const MP4PaytAtom &)
...
#ifdef WIN32
	Error MP4PaytAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4PaytAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4PaytAtom:: createProperties (
...

{
	...
	// 0: PayloadNumber
	if (_mui32pPayloadNumber. init ("PayloadNumber",
	...
	/*
	{
		...
		if ((pRtpMap = new char * [1]) == (char **) NULL)
		{
			Error err = MP4FileErrors (__FILE__, __LINE__,
				MP4F_NEW_FAILED);
			_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
		...
		if ((pRtpMap [0] = new char [1]) == (char *) NULL)
		{
			Error err = MP4FileErrors (__FILE__, __LINE__,
				MP4F_NEW_FAILED);
			_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
		...
		memset (pRtpMap [0], 0, 1);
		if (_mstrcpRtpMap. init ("RtpMap", &bIsImplicitProperty, false, false,
			pRtpMap, _ptTracer) != errNoError)
		{
			Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4PaytAtom:: readProperties (
	unsigned long *pulPropertiesSize)

{
	...
	// 1: RtpMap
	if (_mstrcpRtpMap. init ("RtpMap", false, false, &ulBytesNumber,
...
Error MP4PaytAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui32pPayloadNumber. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mstrcpRtpMap. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4PaytAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					= 0;
	...
	if (_mui32pPayloadNumber. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mstrcpRtpMap. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4PaytAtom:: getType (char *pType)
...
Error MP4PaytAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui32pPayloadNumber. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mstrcpRtpMap. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4PaytAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui32pPayloadNumber. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mstrcpRtpMap. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4PaytAtom:: setRtpMap (Buffer_p pbRtpMap)

{
	...
	if (_mstrcpRtpMap. setValue (pbRtpMap, 0, &lSizeChangedInBytes) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4PaytAtom:: getPayloadNumber (unsigned long *pulPayloadNumber)

{
	...
	if (_mui32pPayloadNumber. getValue (pulPayloadNumber, 0) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4PaytAtom:: setPayloadNumber (unsigned long ulPayloadNumber)

{
	...
	if (_mui32pPayloadNumber. setValue (ulPayloadNumber, 0,
