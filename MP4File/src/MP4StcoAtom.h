...
#ifndef MP4StcoAtom_h
	#define MP4StcoAtom_h

	#include "MP4Atom.h"
	#include "MP4FileErrors.h"


	typedef class MP4StcoAtom: public MP4Atom {
		...
		private:
			...
			MP4UInteger32BitsProperty_t					_mui32pChunkOffsetTable;

		protected:
			MP4StcoAtom (const MP4StcoAtom &);

			MP4StcoAtom &operator = (const MP4StcoAtom &);
		...
		public:
			...
			Error getChunkOffsetTable (unsigned long ulIndex,
				unsigned long *pulChunkOffsetTable);

			Error setChunkOffsetTable (unsigned long ulIndex,
				unsigned long ulChunkOffsetTable);

	} MP4StcoAtom_t, *MP4StcoAtom_p;
