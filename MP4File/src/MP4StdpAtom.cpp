...
MP4StdpAtom:: MP4StdpAtom (void): MP4Atom ()
...
MP4StdpAtom:: MP4StdpAtom (const MP4StdpAtom &)
...
MP4StdpAtom &MP4StdpAtom:: operator = (const MP4StdpAtom &)
...
#ifdef WIN32
	Error MP4StdpAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4StdpAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4StdpAtom:: createProperties (
...

{
	...
	if (_mui32pEntriesNumber. init ("EntriesNumber",
		1, &bIsImplicitProperty, &ulEntriesNumber, _ptTracer) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	// 3: Entries
	// 		priority
	if (_mui16pEntries. init ("Entries", ulEntriesNumber * 1,
...
Error MP4StdpAtom:: readProperties (
	unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize		= 3;
	...
	{
		ulEntriesNumber			=
			((_ullAtomSize - *pulPropertiesSize) / 4);
	}
	else */ if (!strcmp (pParentType, "uuid"))
	{
		ulEntriesNumber			= (unsigned long) ((_ullAtomSize - _ulHeaderSize -
			MP4F_EXTENDEDTYPELENGTH - *pulPropertiesSize) / 4);
	}
	else
	{
		ulEntriesNumber			= (unsigned long)
			((_ullAtomSize - _ulHeaderSize - *pulPropertiesSize) / 4);
	}
...
Error MP4StdpAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui16pEntries. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4StdpAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mui16pEntries. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4StdpAtom:: getType (char *pType)
...
Error MP4StdpAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui16pEntries. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4StdpAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui16pEntries. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
