...
MP4StscAtom:: MP4StscAtom (void): MP4Atom ()
...
MP4StscAtom:: MP4StscAtom (const MP4StscAtom &)
...
MP4StscAtom &MP4StscAtom:: operator = (const MP4StscAtom &)
...
#ifdef WIN32
	Error MP4StscAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4StscAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4StscAtom:: createProperties (
...

{
	...
	// 3: SampleToChunkTable
	// 		FirstChunk / SamplesPerChunk / SampleDescriptionIndex
	if (_mui32pSampleToChunkTable. init ("SampleToChunkTable",
		ulValue * 3, &bIsImplicitProperty, &ulValue, _ptTracer) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_INIT_FAILED);
...
Error MP4StscAtom:: readProperties (
	unsigned long *pulPropertiesSize)

{
	...
	// 		FirstChunk / SamplesPerChunk / SampleDescriptionIndex
	if (_mui32pSampleToChunkTable. init ("SampleToChunkTable",
		ulEntriesNumber * 3, _pfFile, _ptTracer) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_INIT_FAILED);
...
Error MP4StscAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui32pSampleToChunkTable. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_WRITE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StscAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mui32pSampleToChunkTable. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_GETSIZE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StscAtom:: getType (char *pType)
...
Error MP4StscAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui32pSampleToChunkTable. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_APPENDDUMP_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StscAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui32pSampleToChunkTable. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_PRINTONSTDOUTPUT_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StscAtom:: getEntriesNumber (unsigned long *pulEntriesNumber)
...
Error MP4StscAtom:: setEntriesNumber (unsigned long ulEntriesNumber)
...
Error MP4StscAtom:: incrementEntriesNumber (long lIncrement)
...
Error MP4StscAtom:: getSampleToChunkTable (unsigned long ulIndex,
...

{
	...
	if (_mui32pSampleToChunkTable. getValue (pulSampleToChunkTable, ulIndex) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_GETVALUE_FAILED);
...
Error MP4StscAtom:: setSampleToChunkTable (unsigned long ulIndex,
...

{
	...
	if (_mui32pSampleToChunkTable. setValue (ulSampleToChunkTable, ulIndex,
		&ulSizeChangedInBytes) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_SETVALUE_FAILED);
