...
MP4StssAtom:: MP4StssAtom (void): MP4Atom ()
...
MP4StssAtom:: MP4StssAtom (const MP4StssAtom &)
...
MP4StssAtom &MP4StssAtom:: operator = (const MP4StssAtom &)
...
#ifdef WIN32
	Error MP4StssAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4StssAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4StssAtom:: createProperties (
...

{
	...
	// 3: SyncSampleTable
	// 		SampleNumber
	if (_mui32pSyncSampleTable. init ("SyncSampleTable",
...
Error MP4StssAtom:: readProperties (
	unsigned long *pulPropertiesSize)
...
Error MP4StssAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui32pSyncSampleTable. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_WRITE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StssAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mui32pSyncSampleTable. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_GETSIZE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StssAtom:: getType (char *pType)
...
Error MP4StssAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui32pSyncSampleTable. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_APPENDDUMP_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StssAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui32pSyncSampleTable. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4UINTEGER32BITSPROPERTY_PRINTONSTDOUTPUT_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4StssAtom:: getEntriesNumber (unsigned long *pulEntriesNumber)
...
Error MP4StssAtom:: setEntriesNumber (unsigned long ulEntriesNumber)
...
Error MP4StssAtom:: incrementEntriesNumber (long lIncrement)
...
Error MP4StssAtom:: getSyncSampleTable (unsigned long ulIndex,
	unsigned long *pulSyncSampleTable)

{
	...
	if (_mui32pSyncSampleTable. getValue (pulSyncSampleTable, ulIndex) !=
		errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_GETVALUE_FAILED);
...
Error MP4StssAtom:: setSyncSampleTable (unsigned long ulIndex,
	unsigned long ulSyncSampleTable)

{
	...
	if (_mui32pSyncSampleTable. setValue (ulSyncSampleTable, ulIndex,
		&ulSizeChangedInBytes) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4PROPERTY_SETVALUE_FAILED);
