...
*/
...
#ifndef MP4TrpyAtom_h
	#define MP4TrpyAtom_h
	...
	typedef class MP4TrpyAtom: public MP4Atom {

		private:
			MP4UInteger64BitsProperty_t					_mui64pBytes;

		protected:
			MP4TrpyAtom (const MP4TrpyAtom &);

			MP4TrpyAtom &operator = (const MP4TrpyAtom &);
		...
		public:
			...
			#ifdef WIN32
				Error getBytes (__int64 *pullBytes);
			#else
				Error getBytes (unsigned long long *pullBytes);
			#endif

			#ifdef WIN32
				Error setBytes (__int64 ullBytes);
			#else
				Error setBytes (unsigned long long ullBytes);
			#endif
