...
MP4TrunAtom:: MP4TrunAtom (void): MP4Atom ()
...
MP4TrunAtom:: MP4TrunAtom (const MP4TrunAtom &)
...
MP4TrunAtom &MP4TrunAtom:: operator = (const MP4TrunAtom &)
...
#ifdef WIN32
	Error MP4TrunAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4TrunAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4TrunAtom:: createProperties (
...

{
	...
	// 2: sampleCount
	if (_mui32pSampleCount. init ("SampleCount",
...
Error MP4TrunAtom:: readProperties (
	unsigned long *pulPropertiesSize)
...
Error MP4TrunAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mui32pSampleCount. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4TrunAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mui32pSampleCount. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4TrunAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mui32pSampleCount. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4TrunAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mui32pSampleCount. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
...
Error MP4TrunAtom:: getType (char *pType)
