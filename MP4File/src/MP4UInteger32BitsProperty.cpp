...
MP4UInteger32BitsProperty:: MP4UInteger32BitsProperty (void): MP4Property ()
...
MP4UInteger32BitsProperty:: ~MP4UInteger32BitsProperty (void)
...
Error MP4UInteger32BitsProperty:: init (const char *pName,
...

{
	...
	for (lIntegerIndex = 0; lIntegerIndex < _lInstancesNumber; lIntegerIndex++)
	{
		if (pfFile -> readNetUnsignedInt32Bit (&(_pulValues [lIntegerIndex])) !=
			errNoError)
		{
...

{
	...
	for (lIntegerIndex = 0; lIntegerIndex < _lInstancesNumber; lIntegerIndex++)
	{
		...
		_pulValues [lIntegerIndex]			= (
			(pucBuffer [lIntegerIndex * 4] << 24) |
			(pucBuffer [lIntegerIndex * 4 + 1] << 16) |
			(pucBuffer [lIntegerIndex * 4 + 2] << 8) |
			pucBuffer [lIntegerIndex * 4 + 3]);
		*/

		_pulValues [lIntegerIndex]			= ntohl (
			*((unsigned long *) (pucBuffer + (lIntegerIndex * 4))));
	}
...
Error MP4UInteger32BitsProperty:: getValue (unsigned long *pulValue,
	long lIndex)
...
Error MP4UInteger32BitsProperty:: getValue (unsigned char *pucValue,
	long lIndex)

{
	...
	/*
	pucValue [0]			= ((_pulValues [lIndex]) >> 24) & 0xFF;
	pucValue [1]			= ((_pulValues [lIndex]) >> 16) & 0xFF;
	pucValue [2]			= ((_pulValues [lIndex]) >> 8) & 0xFF;
	pucValue [3]			= (_pulValues [lIndex]) & 0xFF;
	*/

	*((unsigned long *) pucValue)			= htonl (_pulValues [lIndex]);
...
Error MP4UInteger32BitsProperty:: setValue (unsigned long ulValue, long lIndex,
...
Error MP4UInteger32BitsProperty:: incrementValue (long lIndex, long lIncrement)
...
Error MP4UInteger32BitsProperty:: write (int iFileDescriptor)
...
Error MP4UInteger32BitsProperty:: getSize (unsigned long *pulSize)
...
Error MP4UInteger32BitsProperty:: getType (char *pType)

{

	strcpy (pType, "UInteger32Bits");
...
Error MP4UInteger32BitsProperty:: appendDump (Buffer_p pbBuffer)
...
Error MP4UInteger32BitsProperty:: printOnStdOutput (void)
