...
MP4UrnAtom:: MP4UrnAtom (void): MP4Atom ()
...
MP4UrnAtom:: MP4UrnAtom (const MP4UrnAtom &)
...
MP4UrnAtom &MP4UrnAtom:: operator = (const MP4UrnAtom &)
...
#ifdef WIN32
	Error MP4UrnAtom:: init (MP4Atom_p pmaParentAtom,
		FileReader_p pfFile,
...
Error MP4UrnAtom:: prepareChildrensAtomsInfo (void)
...
Error MP4UrnAtom:: createProperties (
...

{
	...
	// 2: name
	if (_mstrpName. init ("Name", &bIsImplicitProperty,
	...
	// 3: Location
	if (_mstrpLocation. init ("Location", &bIsImplicitProperty,
		&ulBytesNumber, "", _ptTracer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4STRINGNULLTERMINATEDPROPERTY_INIT_FAILED);
...
Error MP4UrnAtom:: readProperties (
	unsigned long *pulPropertiesSize)

{
	...
	// 2: name
	if (_mstrpName. init ("Name",
		&ulBytesNumber, _pfFile, _ptTracer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4STRINGNULLTERMINATEDPROPERTY_INIT_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
	...
	// 3: Location
	if (_mstrpLocation. init ("Location",
		&ulBytesNumber, _pfFile, _ptTracer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4STRINGNULLTERMINATEDPROPERTY_INIT_FAILED);
...
Error MP4UrnAtom:: writeProperties (int iFileDescriptor)

{
	...
	if (_mstrpName. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mstrpLocation. write (iFileDescriptor) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4STRINGNULLTERMINATEDPROPERTY_WRITE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4UrnAtom:: appendPropertiesDump (Buffer_p pbBuffer)

{
	...
	if (_mstrpName. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mstrpLocation. appendDump (pbBuffer) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4STRINGNULLTERMINATEDPROPERTY_APPENDDUMP_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4UrnAtom:: printPropertiesOnStdOutput (void)

{
	...
	if (_mstrpName. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	if (_mstrpLocation. printOnStdOutput () != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4STRINGNULLTERMINATEDPROPERTY_PRINTONSTDOUTPUT_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4UrnAtom:: getPropertiesSize (unsigned long *pulPropertiesSize)

{
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mstrpName. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
	...
	*pulPropertiesSize					+= ulSize;
	...
	if (_mstrpLocation. getSize (&ulSize) != errNoError)
	{
		Error err = MP4FileErrors (__FILE__, __LINE__,
			MP4F_MP4STRINGNULLTERMINATEDPROPERTY_GETSIZE_FAILED);
		_ptTracer -> trace (Tracer:: TRACER_LERRR, (const char *) err,
...
Error MP4UrnAtom:: getType (char *pType)
