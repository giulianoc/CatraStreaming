...
#ifndef MP4UuidAtom_h
	#define MP4UuidAtom_h

	#include "MP4Atom.h"
	#include "MP4FileErrors.h"


	#define MP4F_EXTENDEDTYPELENGTH				16


	typedef class MP4UuidAtom: public MP4Atom {

		private:
			unsigned char			_pucExtendedType [MP4F_EXTENDEDTYPELENGTH];
			MP4BytesProperty_t		_mbpData;

		protected:
			MP4UuidAtom (const MP4UuidAtom &);

			MP4UuidAtom &operator = (const MP4UuidAtom &);
		...
		public:
			...
			Error getExtendedType (unsigned char *pucExtendedType);

			Error setExtendedType (unsigned char *pucExtendedType);

			Error setData (unsigned char *pucData, unsigned long ulBytesNumber);

	} MP4UuidAtom_t, *MP4UuidAtom_p;
