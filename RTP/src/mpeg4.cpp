	...
	unsigned long* pVopTimeIncrement)
{
	...
	// try {
		...
		unsigned char numSecs = 0;
		while (vop.GetBits(1) != 0) {
			numSecs++;
		}
		vop.SkipBits(1);		// skip marker
		unsigned short numTicks = vop.GetBits(timeBits);
...
unsigned char 
MP4AV_Mpeg4VideoToSystemsProfileLevel(unsigned char videoProfileLevel)
{
...
unsigned char MP4AV_Mpeg4GetVopType(unsigned char* pVopBuf,
	unsigned long vopSize)
{
	unsigned char vopType = 0;
